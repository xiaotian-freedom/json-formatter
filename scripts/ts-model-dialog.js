...
class TypeScriptModelDialog {
    ...
    createDialogElement() {
        ...
        this.overlay.className = 'ts-model-overlay';

        // 创建对话框容器
        this.dialog = document.createElement('div');
        this.dialog.className = 'ts-model-dialog';

        // 添加对话框内容
        this.dialog.innerHTML = `
            <div class="ts-dialog-header">
                <h2>${this.getMessage('tsModelGenerator') || 'TypeScript 模型生成器'}</h2>
                <button id="closeDialogBtn" class="ts-close-btn">
            ...
            <div class="ts-dialog-body">
                <div class="ts-settings-panel">
                    <div class="ts-setting-group">
                        ...
                        <div class="ts-setting-item">
                            <select id="unknownType" class="ts-setting-select">
    ...
    bindEvents() {
        ...
        // 监听设置变更事件
        const settingsPanel = this.dialog.querySelector('.ts-settings-panel');
        if (settingsPanel) {
    ...
    regenerate() {
        // 保存当前的滚动位置
        const codeEditor = this.dialog.querySelector('.ts-code-editor');
     ...
     */
    playSiriAnimation() {
        const codeContainer = this.dialog.querySelector('.ts-code-container');
        if (!codeContainer) return;

        // 检查是否已有动画元素
        let siriEffect = codeContainer.querySelector('.ts-siri-effect');
        ...
        // 移除旧的动画元素
        const oldWave = siriEffect.querySelector('.ts-siri-wave');
        if (oldWave) {
            oldWave.remove();
        }
        ...
        newWave.className = 'ts-siri-wave';
        siriEffect.appendChild(newWave);

        // 激活动画
        codeContainer.classList.add('ts-siri-active');

        // 动画结束后移除类
        setTimeout(() => {
            codeContainer.classList.remove('ts-siri-active');
        }, 2000);
    }
    ...
    saveSettings() {
        ...
        try {
            localStorage.setItem('ts-model-settings', JSON.stringify(settings));
        } catch (e) {
    ...
    restoreSettings() {
        try {
            const savedSettings = localStorage.getItem('ts-model-settings');
            if (savedSettings) {
